...
func TestCategoryService_CreateCategory(t *testing.T) {
	t.Run("successful creation", func(t *testing.T) {
		...
		idGen := &FixedIDGenerator{IDs: []string{"uuid-1", "uuid-2", "uuid-3"}}
	service := NewCategoryService(mockStorage, idGen, validate, config)
		...
		assert.Equal(t, "uuid-1", category.ID) // UUID generated by FixedIDGenerator
		...
		// Verify stored
		stored, err := mockStorage.GetCategory(context.Background(), "uuid-1")
		require.NoError(t, err)
